...
BEGIN
    PUBLIC PROCEDURE UPDATE_FECHAS_WEBI (IN i_PeriodSeq BIGINT, IN i_tabla_informe NVARCHAR(255)) 
        ...
        SQL SECURITY INVOKER AS 
          BEGIN 
            ...
            USING (SELECT PER.NAME AS periodo, :i_PeriodSeq AS periodseq, :i_tabla_informe AS tabla 
              FROM CS_PERIOD PER 
              WHERE PER.PERIODSEQ = :i_PeriodSeq AND PER.REMOVEDATE = TO_DATE('2200-01-01','YYYY-MM-DD') ) AS CONDICION 
                ON FEC_WEBI.PERIODSEQ = CONDICION.periodseq AND FEC_WEBI.tabla = CONDICION.tabla 
            WHEN MATCHED THEN UPDATE 
              SET FEC_WEBI.ULTIMA_ACTUALIZACION = UTCTOLOCAL(CURRENT_TIMESTAMP, 'CET') 
            WHEN NOT MATCHED THEN INSERT 
              (PERIODO, TABLA, ULTIMA_ACTUALIZACION, DESCRIPCION, PERIODSEQ) VALUES ( CONDICION.periodo, CONDICION.tabla, UTCTOLOCAL(CURRENT_TIMESTAMP, 'CET'), 'Ultima carga de la tabla ' || :i_tabla_informe, CONDICION.periodseq ); 
    END;
    ...
    PUBLIC PROCEDURE w_stop (OUT v_stop NUMBER(1)) 
        ...
        SQL SECURITY INVOKER AS 
          BEGIN 
            SELECT GC.GENERICBOOLEAN1 INTO v_stop 
            ...
            INNER JOIN CS_CATEGORYTREE CT ON CCC.CATEGORYTREESEQ = CT.CATEGORYTREESEQ 
              ...
              AND CT.ISLAST = 1 
            WHERE CT.NAME = 'Permisos' 
              AND C.CLASSIFIERID = 'RECARGA' 
              AND C.REMOVEDATE = TO_DATE('22000101','YYYYMMDD') 
              AND C.ISLAST = 1; 
    END;
    
    PUBLIC FUNCTION n_meses(n INTEGER) 
      RETURNS TABLE (PERIODSEQ BIGINT) 
        /*--------------------------------------------------------------------- 
        | Author: Brais Romero García 
        | Company: Inycom 
        | Initial Version Date: 01-Diciembre-2025 
        |---------------------------------------------------------------------- 
        | Procedure Purpose: Procedimiento para devolver el PERIODSEQ desde Enero 2025 
        | Version: 	0.1 BRG 20250521 Initial Version. 
		    |			      0.2	BRG	20260226 Ahora devuelve los PERIODSEQ desde Enero 2025, por petición de Manuel Araque
        | ----------------------------------------------------------------------- 
        */ 
        LANGUAGE SQLSCRIPT AS 
          BEGIN 
            DECLARE FEC_ACTUAL TIMESTAMP := ADD_DAYS(CURRENT_DATE, 10); 
            RETURN 
              SELECT PER.PERIODSEQ 
              FROM CS_PERIOD PER 
              INNER JOIN CS_PERIODTYPE PT ON PER.PERIODTYPESEQ = PT.PERIODTYPESEQ 
                AND PT.REMOVEDATE = TO_DATE('22000101','YYYYMMDD') 
                AND PER.REMOVEDATE = TO_DATE('22000101','YYYYMMDD') 
                AND PT.NAME = 'month' 
              WHERE PER.STARTDATE <= FEC_ACTUAL AND PER.STARTDATE > '2024-12-31'; 
    END;
END