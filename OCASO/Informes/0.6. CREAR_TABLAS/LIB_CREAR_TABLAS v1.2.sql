...
BEGIN
 ...
 /*---------------------------------------------------------------------
  ...
  |				1.2 BRG 20251201 Nueva función trece_meses para sacar el PERIODSEQ del mes actual y los 12 anteriores
    -----------------------------------------------------------------------
 */
 LANGUAGE SQLSCRIPT
 ...
 */
 ...
 END;
 PUBLIC FUNCTION trece_meses() RETURNS TABLE (PERIODSEQ INTEGER)
 /*---------------------------------------------------------------------
    | Author: Brais Romero García
    | Company: Inycom
    | Initial Version Date: 01-Diciembre-2025
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento para devolver el PERIODSEQ del mes actual y los 12 anteriores
	| Version: 0.1 BRG 20250521 Initial Version.
	|
     -----------------------------------------------------------------------
 */
	...
	BEGIN
		RETURN
			...
			INNER JOIN CS_PERIODTYPE PT ON PER.PERIODTYPESEQ = PT.PERIODTYPESEQ
				...
				AND PT.NAME = 'month'
			WHERE PER.STARTDATE <= CURRENT_DATE
				AND PER.STARTDATE > ADD_MONTHS(CURRENT_DATE, -13);
	END;
END