CREATE PROCEDURE EXT.SP_RELLENAR_TABLAS_MANUAL ( OUT o_file_name VARCHAR(120), IN i_fecha VARCHAR(8), IN i_listado_informes VARCHAR(5000))
LANGUAGE SQLSCRIPT 
...
BEGIN
	...
	BEGIN
		...
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Parámetros de entrada -> i_fecha: ' || i_fecha || ' | i_listado_informes: ' || i_listado_informes, v_log_count, v_idproceso, 'info');
		...
		--Obtener periodSeq a partir del i_fecha
		--PENDIENTE
		CALL LIB_GLOBAL:getPeriodSeqFromDate(v_idtenant, i_fecha, 'Main Monthly Calendar', 'month', v_periodSeq);
