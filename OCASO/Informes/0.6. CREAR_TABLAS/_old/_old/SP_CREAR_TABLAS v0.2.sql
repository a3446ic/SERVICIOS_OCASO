...
BEGIN
	...
	-- uso de variables de tabla si es posible
		...
		---------------------------------------------------------------------------------------------------------------------------------
		-- DTB 20250522: Tabla temporal TEMP_LIQEXT_PERCEPCIONES_FILE pasa a ser TBL_TABLA_PERCEPCIONES
		...
		IF v_periodo_posteado = 0 THEN
			...
			CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Inicio modificacion EXT.CARTERA_DDEE si ya no es derechos economicos.', v_log_count, v_idproceso, 'debug');
			...
			CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin modificacion EXT.CARTERA_DDEE si ya no es derechos economicos. Filas: ' || v_num_rows, v_log_count, v_idproceso, 'debug');
		...
		TBL_AGENTES_PRIN_MES = SELECT DISTINCT 
            ...
            INNER JOIN TCMP.CSA_PERIODDIMENSION PD ON PAD_INS.PERIODSEQ = PD.PERIODSEQ
                AND PD.TENANTID = :v_idtenant
            LEFT OUTER JOIN (
				SELECT PR.PARENTPOSITIONSEQ,PR.CHILDPOSITIONSEQ,PR.EFFECTIVESTARTDATE,PR.EFFECTIVEENDDATE 
            ...
            ) PR ON PAD_INS.POSITIONSEQ = PR.CHILDPOSITIONSEQ 
    ...
    -- INICIO TABLAS DE INFORMES
		...
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Inicio Borrado OUT_JERARQUIA_REP', v_log_count, v_idproceso, 'debug');

		DELETE EXT.OUT_JERARQUIA_REP WHERE PERIODSEQ = :i_PeriodSeq;

		v_num_rows := ::ROWCOUNT;

		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin Borrado OUT_JERARQUIA_REP. Filas borradas: ' || v_num_rows, v_log_count, v_idproceso, 'debug');

		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Inicio Carga OUT_JERARQUIA_REP', v_log_count, v_idproceso, 'debug');
		...
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin Carga OUT_JERARQUIA_REP. Filas: ' || v_num_rows, v_log_count, v_idproceso, 'debug');
    -------------------------------------------------------------------------------------------------------------------------------------
