CREATE PROCEDURE EXT.SP_PYGINF_EA(OUT FILENAME VARCHAR(120), IN i_param VARCHAR(50))
LANGUAGE SQLSCRIPT  
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Generacion datos para el fichero PyG Eterna
 |
...
BEGIN
 ...
 DECLARE v_file_name VARCHAR(20) := 'PYGINF2082_';
 ...
 END;
    ...
    --------------- Truncado de OUT_REPEXT_INF_PYG_EA_FILE -------------- 
    CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Inicio Truncado de la tabla de OUT_REPEXT_INF_PYG_EA_FILE.', v_log_count, v_idproceso, 'info');
    TRUNCATE TABLE EXT.OUT_REPEXT_INF_PYG_EA_FILE;
    CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Fin Truncado de la tabla de OUT_REPEXT_INF_PYG_EA_FILE.', v_log_count, v_idproceso, 'info');
    
    --------------- Creacion de OUT_REPEXT_INF_PYG_EA_FILE --------------
 CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Inicio Carga de la tabla OUT_REPEXT_INF_PYG_EA_FILE.', v_log_count, v_idproceso, 'info'); 
 
 INSERT INTO EXT.OUT_REPEXT_INF_PYG_EA_FILE
  SELECT LINE
  FROM EXT.FINAL_REPEXT_INF_PyG_FILE P
  WHERE SUBSTR(P.LINE,1,2) = '03'
   AND P.PERIODSEQ = :v_PeriodSeq
  ORDER BY SUBSTR(LINE,1,2), SUBSTR(LINE,64,4), SUBSTR(LINE,96,4), SUBSTR(LINE,44,5), SUBSTR(LINE,25,19);
 
 v_num_rows = RECORD_COUNT(EXT.OUT_REPEXT_INF_PYG_EA_FILE);
 
 
 CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Fin Carga de la tabla OUT_REPEXT_INF_PYG_EA_FILE: ' || v_num_rows || ' filas.', v_log_count, v_idproceso, 'info');
