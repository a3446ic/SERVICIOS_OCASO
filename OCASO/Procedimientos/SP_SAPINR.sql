CREATE PROCEDURE EXT.SP_SAPINR(OUT FILENAME VARCHAR(120), IN i_param VARCHAR(50))
LANGUAGE SQLSCRIPT
...
BEGIN
 ...
 DECLARE v_file_name VARCHAR(20) := 'SAPINR2109_';
...
-- SELECT v_file_name||TO_VARCHAR(v_PeriodStartDate, 'YYYYMM')||'_'||TO_VARCHAR(CURRENT_DATE, 'YYYYMMDD')||'_'||TO_VARCHAR(ADD_SECONDS(CURRENT_TIME, 7200), 'HH24MISS')||'.txt' INTO FILENAME from dummy;
 ...
 END;
    ...
    --------------- Truncado de OUT_REPEXT_INR_SAP_FILE -------------- 
    CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Inicio Truncado de la tabla de OUT_REPEXT_INR_SAP_FILE.', v_log_count, v_idproceso, 'info');
    TRUNCATE TABLE EXT.OUT_REPEXT_INR_SAP_FILE;
    CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Fin Truncado de la tabla de OUT_REPEXT_INR_SAP_FILE.', v_log_count, v_idproceso, 'info');
    
    --------------- Creacion de OUT_REPEXT_INR_SAP_FILE --------------
 CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Inicio Carga de la tabla OUT_REPEXT_INR_SAP_FILE.', v_log_count, v_idproceso, 'info'); 
 
 INSERT INTO EXT.OUT_REPEXT_INR_SAP_FILE
  SELECT P.LINE
  FROM EXT.FINAL_REPEXT_INR_SAP_FILE P
 ...
 v_num_rows = RECORD_COUNT(EXT.OUT_REPEXT_INR_SAP_FILE);
 CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Fin Carga de la tabla OUT_REPEXT_INR_SAP_FILE: ' || v_num_rows || ' filas.', v_log_count, v_idproceso, 'info');
