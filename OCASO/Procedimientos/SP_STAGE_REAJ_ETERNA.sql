CREATE PROCEDURE EXT.SP_STAGE_REAJ_ETERNA (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para REAJUSTES ETERNA a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_REAJ_ETERNA a STAGE_REAJUSTES
	|
	| Version: 0.1	BRG 20250124	Initial Version.
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		--Llamamos a la libreria global para realizar la insercion a la stage_REAJUSTES
		CALL ext.lib_global:SP_insertaStageReajustes (v_permisos_log,  proc_name , 'EXT.PRESTAGE_REAJ_ETERNA', i_file_name ,v_log_count, v_idproceso);
		...
		CALL EXT.LIB_GLOBAL:WRITE_LOG(v_permisos_log,proc_name, 'Llamada a SP_POPULATE_REAJUSTES para ' || i_file_name, v_log_count, v_idproceso, 'info');
		
		CALL EXT.SP_POPULATE_REAJUSTES(i_file_name,v_idproceso, v_log_count);
