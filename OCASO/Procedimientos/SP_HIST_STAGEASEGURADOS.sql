CREATE PROCEDURE EXT.SP_HIST_STAGEASEGURADOS (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para historicos de STAGE_ASEGURADOS a traves del CDL
    |
...
BEGIN
	...
	--Inicializamos el idProceso
		...
		FROM EXT.PRESTAGE_HIST_STAGEASEGURADOS;
		...
		SELECT DISTINCT :v_id_proceso 
			, :i_file_name
			, HIST.FILE_NAME
		FROM EXT.PRESTAGE_HIST_STAGEASEGURADOS HIST;
		...
		/*	
		DELETE FROM EXT.STAGE_ASEGURADOS REC
		WHERE FILE_NAME IN (SELECT FILE_NAME_LP FROM :TBL_EJEC_HIST_CONTROL);
			
		v_num_rows = ::rowcount;
		
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin DELETE en STAGE_ASEGURADOS. Filas: ' || v_num_rows, v_log_count, v_id_proceso, 'info');
		*/
		INSERT INTO EXT.STAGE_ASEGURADOS
		SELECT HIST.FILE_NAME
				, TO_INTEGER(HIST.STATUS)
				, CURRENT_TIMESTAMP
				, HIST.CODIGOPOLIZA
				, HIST.NUMEROASEGURADO
				, HIST.CODIGORECIBO
				, HIST.NIF
				, TO_DATE(HIST.FECHANACIMIENTO,'DD/MM/YYYY')
				, TO_DATE(HIST.FECHADERECHOS,'DD/MM/YYYY')
				, HIST.MOTIVOALTA
				, TO_DATE(HIST.FECHAALTA,'DD/MM/YYYY')
				, HIST.MOTIVOBAJA
				, TO_DATE(HIST.FECHABAJA,'DD/MM/YYYY')
				, TO_DATE(HIST.FECHAREHABILITACION,'DD/MM/YYYY')
				, HIST.SUBTIPOMOVIMIENTO
				, HIST.PRODUCTOCONTABLE
				, TO_DATE(HIST.FECHAALTAGARASE,'DD/MM/YYYY')
				, TO_DATE(HIST.FECHABAJAGARASE,'DD/MM/YYYY')
				, HIST.PRIMAUNICA
				, HIST.GARANTIAIP
				, HIST.PRIMANETAASEGURADO
				, HIST.CAPITALNATURAL
				, HIST.CAPITALNIVELADO
				, HIST.POLIZAORIGEN
				, HIST.ASEGURADOORIGEN
		FROM EXT.PRESTAGE_HIST_STAGEASEGURADOS HIST
		;
		
		v_num_rows = ::rowcount;
		
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin INSERT en STAGE_ASEGURADOS. Filas: ' || v_num_rows, v_log_count, v_id_proceso, 'info');
