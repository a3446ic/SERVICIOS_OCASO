CREATE PROCEDURE EXT.SP_STAGE_RECI_GESTIONES (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para RECIBOS GESTIONES a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_RECI_GESTIONES a STAGE_RECIBOS
	|
...
BEGIN
	...
	BEGIN
		...
		CALL ext.lib_global:SP_insertaStageRecibosGestiones (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_GESTIONES', i_file_name ,v_log_count, v_idproceso);
		...
		CALL EXT.LIB_GLOBAL:WRITE_LOG(v_permisos_log,proc_name, 'Llamada a SP_POPULATE_GESTIONES para ' || i_file_name, v_log_count, v_idproceso, 'info');
		
		CALL EXT.SP_POPULATE_GESTIONES(i_file_name,v_idproceso, v_log_count);
