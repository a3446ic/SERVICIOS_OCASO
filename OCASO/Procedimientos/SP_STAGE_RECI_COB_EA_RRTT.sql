CREATE PROCEDURE EXT.SP_STAGE_RECI_COB_EA_RRTT (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para RECIBOS COBRADO DE RRTT ETERNA a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_RECI_COB_EA_RRTT a STAGE_RECIBOS
	|
	| Version: 0.1	TGV 20250123	Initial Version.
	| Version: 0.2	ITL 20250408	Added call to SP_POPULATE_COBRADO_RRTT (line 74)
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		CALL ext.lib_global:SP_insertaStageRecibos (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_COB_EA_RRTT', i_file_name ,v_log_count, v_idproceso);
