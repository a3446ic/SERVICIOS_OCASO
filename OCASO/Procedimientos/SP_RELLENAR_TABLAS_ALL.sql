CREATE PROCEDURE EXT.SP_RELLENAR_TABLAS_ALL ( OUT FILENAME VARCHAR(120), IN iPipelineRunSeq BIGINT)
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    | Author: Brais Romero
    | Company: Inycom
    | Initial Version Date: 13-Noviembre-2025
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento para la recarga de tablas FINAL de informes. 
	...
	|
	| Version:	0.1	BRG 20251113	Initial Version.
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		--FILENAME = 'OUT_RELLENAR_TABLAS_ALL_'||TO_VARCHAR(CURRENT_TIMESTAMP,'YYYYMMDD_HH24MISS')||'.txt';
		--BRG 20250814 Cambiamos nombres de ficheros por petición de MAA
		FILENAME = 'OUT_RELLENAR_TABLAS_ALL_'||TO_VARCHAR(CURRENT_DATE, 'YYYYMMDD')||'_'||TO_VARCHAR(ADD_SECONDS(CURRENT_TIME, 7200), 'HH24MISS')||'.txt';
		...
		VALUES (v_idproceso, FILENAME, proc_name, 0, v_const_out_batch_control_load, CURRENT_TIMESTAMP, NULL);
		...
		CALL EXT.SP_RELLENAR_TABLAS(v_periodSeq, 'ALL', v_idproceso, v_log_count);
