CREATE PROCEDURE EXT.SP_STAGE_RECI_COB_OC_RRTT (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para RECIBOS COBRADO OCASO RRTT a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_RECI_COB_OC_RRTT a STAGE_RECIBOS
	|
	| Version: 0.1	ITL 20250123	Initial Version.
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		FROM EXT.PRESTAGE_RECI_COB_OC_SOLNET;
		
		INSERT INTO EXT.IN_BATCH_CONTROL (ID_PROCESO, FILE_NAME, PROCEDURE_NAME, SOURCE_ROWS, STATUS, START_DATE)
		VALUES (v_idproceso, i_file_name, proc_name, v_num_rows, v_const_prestage_status_load, CURRENT_TIMESTAMP);
		
		COMMIT;
		--Llamamos a la libreria global para realizar la insercion a la stage_recibos
		CALL ext.lib_global:SP_insertaStageRecibos (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_COB_OC_RRTT', i_file_name ,v_log_count, v_idproceso);
		...
		CALL EXT.LIB_GLOBAL:WRITE_LOG(v_permisos_log,proc_name, 'Llamada a SP_POPULATE_COBRADO_RRTT para ' || i_file_name, v_log_count, v_idproceso, 'info');
		
		CALL EXT.SP_POPULATE_COBRADO_RRTT(i_file_name,v_idproceso, v_log_count);
