CREATE PROCEDURE EXT.SP_STAGE_RECI_CAR_OC_SOLNET (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para RECIBOS CARTERA DE OCASO SOLNET a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_RECI_CAR_OC_SOLNET a STAGE_RECIBOS
	|
	| Version: 0.1	BRG 20250123	Initial Version.
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		FROM EXT.PRESTAGE_RECI_CAR_OC_SOLNET;
		...
		CALL ext.lib_global:SP_insertaStageRecibos (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_CAR_OC_SOLNET', i_file_name ,v_log_count, v_idproceso);
		
		--Actualizamos la IN_BATCH_CONTROL y ponemos el estado a 1
	--	UPDATE EXT.IN_BATCH_CONTROL SET STATUS = v_in_batch_load, END_DATE = CURRENT_TIMESTAMP WHERE FILE_NAME = i_file_name AND ID_PROCESO = v_idproceso;
