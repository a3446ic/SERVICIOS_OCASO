CREATE PROCEDURE EXT.SP_STAGE_RECI_EMI_EA_RRGG (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para RECIBOS EMITIDOS DE RRGG ETERNA a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_RECI_EMI_EA_RRGG a STAGE_RECIBOS
	|
...
BEGIN
	...
	BEGIN
		...
		FROM EXT.PRESTAGE_RECI_EMI_EA_RRGG;
		...
		CALL ext.lib_global:SP_insertaStageRecibos (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_EMI_EA_RRGG', i_file_name ,v_log_count, v_idproceso);
