CREATE PROCEDURE EXT.SP_STAGE_REAJ_OCASO (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para REAJUSTES OCASO a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_REAJ_OCASO a STAGE_REAJUSTES
	|
	| Version: 0.1	DTB 20250124	Initial Version.
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		CALL ext.lib_global:SP_insertaStageReajustes ( v_permisos_log, proc_name , 'EXT.PRESTAGE_REAJ_OCASO', i_file_name ,v_log_count, v_idproceso);
