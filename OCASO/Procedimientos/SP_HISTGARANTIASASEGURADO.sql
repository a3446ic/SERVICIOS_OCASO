CREATE PROCEDURE EXT.SP_HISTGARANTIASASEGURADO (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para historicos de GARANTIASASEGURADO a traves del CDL
    |
...
BEGIN
	...
	--Inicializamos el idProceso
		...
		FROM EXT.PRESTAGE_HISTGARANTIASASEGURADO;
		...
		SELECT DISTINCT :v_id_proceso 
			, :i_file_name
			, HIST.FILE_NAME
		FROM EXT.PRESTAGE_HISTGARANTIASASEGURADO HIST;
		...
		/*	
		DELETE FROM EXT.GARANTIAS_ASEGURADO GAR_ASE
		WHERE FILE_NAME IN (SELECT FILE_NAME_LP FROM :TBL_EJEC_HIST_CONTROL);
			
		v_num_rows = ::rowcount;
		
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin DELETE en GARANTIAS_ASEGURADO. Filas: ' || v_num_rows, v_log_count, v_id_proceso, 'info');
		*/
		--ITL cambiado orden columnas 
		INSERT INTO EXT.GARANTIAS_ASEGURADO
		SELECT -99999999 AS ID_ASEGURADO
				, -99999999 AS ID_REAJUSTE
				, HIST.FILE_NAME
				...
				, HIST.PRIMA_UNICA
				, HIST.GARANTIA_IP
				, TO_DECIMAL(REPLACE(HIST.PRIMA_NETA_ASEGURADO,',','.'),15,2)
				, TO_DECIMAL(REPLACE(HIST.PORCENTAJE_PARTICIPACION,',','.'),15,2)
				, TO_DECIMAL(REPLACE(HIST.PORCENTAJE_NIVELADA,',','.'),15,2)
				, TO_DECIMAL(REPLACE(HIST.CAPITAL_NATURAL,',','.'),15,2)
				, TO_DECIMAL(REPLACE(HIST.CAPITAL_NIVELADO,',','.'),15,2)
				, HIST.SUBTIPO_MOVIMIENTO
				, HIST.MARCA_CUENTA
				...
				, TO_DATE(HIST.FECHA_ALTA_GAR_ASE,'DD/MM/YYYY')
				, TO_DATE(HIST.FECHA_BAJA_GAR_ASE,'DD/MM/YYYY')
				, HIST.NUM_ORDEN_MOVIMIENTO
				, TO_DECIMAL(REPLACE(HIST.PC_PERIODO,',','.'),15,2)
				, TO_DECIMAL(REPLACE(HIST.NUM_PERIODOS,',','.'),15,2)
		FROM EXT.PRESTAGE_HISTGARANTIASASEGURADO HIST
		;
		
		v_num_rows = ::rowcount;
		
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Fin INSERT en GARANTIAS_ASEGURADO. Filas: ' || v_num_rows, v_log_count, v_id_proceso, 'info');
