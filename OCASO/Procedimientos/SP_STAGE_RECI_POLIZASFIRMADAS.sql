CREATE PROCEDURE EXT.SP_STAGE_RECI_POLIZASFIRMADAS (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    | Company: Inycom
    | Initial Version Date: 30-Mayo-2025
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para POLIZAS FIRMADAS
	|
...
BEGIN
	...
	BEGIN
		...
		CALL ext.lib_global:SP_insertaStageRecibosGestiones (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_POLFIRMADAS', i_file_name ,v_log_count, v_idproceso);
		...
		CALL EXT.LIB_GLOBAL:WRITE_LOG(v_permisos_log,proc_name, 'Llamada a SP_POPULATE_POLIZASFIRMADAS para ' || i_file_name, v_log_count, v_idproceso, 'info');
		
		CALL EXT.SP_POPULATE_POLIZASFIRMADAS(i_file_name,v_idproceso, v_log_count);
