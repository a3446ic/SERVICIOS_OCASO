CREATE PROCEDURE EXT.SP_STAGE_RECI_COB_EA_SERCO_RRGG (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para RECIBOS COBRADO DE RRGG ETERNA SERCO a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_RECI_COB_EA_SERCO_RRGG a STAGE_RECIBOS
	|
...
BEGIN
	...
	BEGIN
		...
		CALL ext.lib_global:SP_insertaStageSerco (v_permisos_log, proc_name , 'EXT.PRESTAGE_RECI_COB_EA_SERCO_RRGG', i_file_name ,v_log_count, v_idproceso);
