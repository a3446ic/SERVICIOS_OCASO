CREATE OR REPLACE PROCEDURE EXT.SP_CREAR_TABLAS_MANUAL ( OUT o_file_name VARCHAR(120), IN i_fechas VARCHAR(5000), IN i_listado_informes VARCHAR(5000), IN i_tipo INTEGER DEFAULT NULL)
LANGUAGE SQLSCRIPT 
...
/*---------------------------------------------------------------------
	...
	|			0.2	BRG	20260112	Adaptado para permitir múltiples fechas
	|			0.3	BRG	20251212	Añadido i_tipo para controlar el tipo de ejecución del periodo (CIERRE, LC o BAL)
	|
    -----------------------------------------------------------------------
*/
...
BEGIN
	...
	BEGIN
		...
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Parámetros de entrada -> i_fechas: ' || i_fechas || ' | i_listado_informes: ' || i_listado_informes || ' | i_tipo: ' || IFNULL(TO_VARCHAR(i_tipo), 'NULL'), v_log_count, v_idproceso, 'info');
		...
		CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Bucle para extraer cada fecha de i_fechas', v_log_count, v_idproceso, 'info');
		WHILE current_index < len_fechas DO
			SELECT SUBSTR(i_fechas, current_index, 8) INTO v_fecha FROM DUMMY;
			CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, proc_name, 'Inicio ejecución CREAR_TABLAS para v_fecha = ' || v_fecha, v_log_count, v_idproceso, 'info');
			...
			CALL EXT.SP_CREAR_TABLAS(v_periodseq, i_listado_informes, v_idproceso, i_tipo, v_log_count);
