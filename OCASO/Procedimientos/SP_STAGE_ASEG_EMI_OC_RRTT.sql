CREATE PROCEDURE EXT.SP_STAGE_ASEG_EMI_OC_RRTT (IN i_file_name varchar(120))
LANGUAGE SQLSCRIPT 
SQL SECURITY INVOKER 
DEFAULT SCHEMA EXT AS

/*---------------------------------------------------------------------
    | Author: Tania Garces Villanueva
    | Company: Inycom
    | Initial Version Date: 24-Enero-2025
    |----------------------------------------------------------------------
    | Procedure Purpose: Procedimiento que se ejecuta cuando se sube un fichero para DIARIO OCASO DE RRTT (ASEGURADOS) a traves del CDL, 
    | y que mueve los datos de la tabla PRESTAGE_ASEG_EMI_EA_RRTT a STAGE_ASESEGURADOS
	|
	| Version: 0.1	TGV 20250124	Initial Version.
	| Version: 0.2  ITL 20250219	Added Call to POPULATE_ASEGURADOS & update IN_BATCH_CONTROL
	|
    -----------------------------------------------------------------------
*/
...
USING SQLSCRIPT_STRING AS LIBRARY;
	...
	BEGIN
		...
		FROM EXT.PRESTAGE_RECI_EMI_ASISA;
		...
		CALL ext.lib_global:SP_insertaStageAsegurados (v_permisos_log, proc_name , 'EXT.PRESTAGE_ASEG_EMI_OC_RRTT', i_file_name ,v_log_count, v_idproceso);
		...
		CALL EXT.LIB_GLOBAL:WRITE_LOG(v_permisos_log,proc_name, 'Llamada a SP_POPULATE_ASEGURADOS para ' || i_file_name, v_log_count, v_idproceso, 'info');
		
		CALL EXT.SP_POPULATE_ASEGURADOS(i_file_name,v_idproceso, v_log_count);
