CREATE PROCEDURE EXT.SP_CDAINF_OC(OUT FILENAME VARCHAR(120), IN i_param VARCHAR(50))
LANGUAGE SQLSCRIPT
...
/*---------------------------------------------------------------------
    ...
    |----------------------------------------------------------------------
    | Procedure Purpose: Generacion datos para el fichero CDA Ocaso
 |
 ...
 |   0.5 SMM 20250911  Cambio parámetros de entrada
 |
    -----------------------------------------------------------------------
*/
BEGIN
 ...
 DECLARE v_file_name VARCHAR(20) := 'CDAINF2079_';
 DECLARE v_pos1 NVARCHAR(12);
    DECLARE v_pos2 NVARCHAR(12);
    DECLARE i_fecha VARCHAR(8);
    DECLARE i_ejecucion VARCHAR(1);
 ...
 DECLARE v_const_out_batch_control_load INTEGER := EXT.LIB_CONSTANTES :CONST_OUT_BATCH_CONTROL_LOAD;
    ...
    IF EXISTS( SELECT 1 FROM DUMMY WHERE INSTR(i_param, ';') > 0) THEN
     -- Posición del primer separador
     v_pos1 := INSTR(i_param, ';');
     

     -- Extraer los valores
     i_fecha := SUBSTRING(i_param, 1, v_pos1 - 1);
     i_ejecucion := SUBSTRING(i_param, v_pos1 + 1);
     CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'i_fecha: ' || i_fecha || ' - i_ejecucion ' || :i_ejecucion , v_log_count, v_idproceso, 'info');
  --Función para sacar periodo a partir de la fecha
  
  SELECT PERIODSEQ, NAME, STARTDATE, TO_VARCHAR(STARTDATE,'YYYYMM') INTO v_PeriodSeq, v_PeriodName, v_PeriodStartDate,v_fechaliquidacion FROM EXT.LIB_GLOBAL:getPeriodRowFromFecha(:v_idtenant, :i_fecha);
  -- IF :i_ejecucion = 1 THEN
   v_file_name  := 'CDAPRO2107_';
  -- END if;
  ELSE
      -- 20260109 -- TGV -- Cambiamos la forma de obtener el periodo
      ...
      -------------------------------------------------------------------------------
	    ...
	    IF ( (SELECT LENGTH(:i_param) FROM DUMMY ) > 6) THEN
			...
			SELECT PERIODSEQ, NAME, STARTDATE, TO_VARCHAR(STARTDATE,'YYYYMM') INTO v_PeriodSeq, v_PeriodName, v_PeriodStartDate,v_fechaliquidacion 
                FROM EXT.LIB_GLOBAL:getPeriodRow(:v_idTenant, :i_param);
 ...
 END;
    ...
    --------------- Truncado de OUT_REPEXT_CDA_OC_FILE -------------- 
    CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Inicio Truncado de la tabla de OUT_REPEXT_CDA_OC_FILE.', v_log_count, v_idproceso, 'info');
    TRUNCATE TABLE EXT.OUT_REPEXT_CDA_OC_FILE;
    CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Fin Truncado de la tabla de OUT_REPEXT_CDA_OC_FILE.', v_log_count, v_idproceso, 'info');
    
    --------------- Creacion de OUT_REPEXT_CDA_OC_FILE --------------
 CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Inicio Carga de la tabla OUT_REPEXT_CDA_OC_FILE.', v_log_count, v_idproceso, 'info'); 
 
 INSERT INTO EXT.OUT_REPEXT_CDA_OC_FILE
  SELECT P.LINE
  FROM EXT.FINAL_REPEXT_CDA_FILE P
  WHERE SUBSTR(P.LINE,1,2) = '01'
   AND P.PERIODSEQ = :v_PeriodSeq
  ORDER BY CASE WHEN P.LINE IS NULL OR P.LINE = '' THEN 1 ELSE 0 END,
  SUBSTR(P.LINE,1,2), SUBSTR(P.LINE,56,10), SUBSTR(P.LINE,25,4), SUBSTR(P.LINE,80,5), SUBSTR(P.LINE,85,19);
 
 v_num_rows = RECORD_COUNT(EXT.OUT_REPEXT_CDA_OC_FILE);
 CALL LIB_GLOBAL:WRITE_LOG (v_permisos_log, v_proc_name, 'Fin Carga de la tabla OUT_REPEXT_CDA_OC_FILE: ' || v_num_rows || ' filas.', v_log_count, v_idproceso, 'info');
